...
function populate() {
  ...
  // Reconstruct tiposConfig from variantes
  const tiposMap = {}
  const variantes = p.variantes || []
  for (const v of variantes) {
    if (!v.tipoProducto) continue
    if (!tiposMap[v.tipoProducto]) {
      tiposMap[v.tipoProducto] = { nombre: v.tipoProducto, terminaciones: [] }
    }
    if (v.terminacion && !tiposMap[v.tipoProducto].terminaciones.includes(v.terminacion)) {
      tiposMap[v.tipoProducto].terminaciones.push(v.terminacion)
    }
  }
  const tiposConfig = Object.values(tiposMap)
  if (tiposConfig.length === 0) {
    tiposConfig.push({ nombre: '', terminaciones: [] })
  }

  // Add any terminaciones from the product's variantes that aren't in DEFAULT_TERMINACIONES
  for (const v of variantes) {
    if (v.terminacion && !allTerminaciones.value.some(t => t.code === v.terminacion)) {
      allTerminaciones.value.push({ nombre: v.tipoTerminacion || v.terminacion, code: v.terminacion })
    }
  }
  ...
  for (const v of variantes) {
    if (!v.tipoProducto || !v.terminacion) continue
    const key = `${v.tipoProducto}|${v.terminacion}`
    const termDef = allTerminaciones.value.find(t => t.code === v.terminacion)
    priceRows.value[key] = {
      _key: key,
      tipoProducto: v.tipoProducto,
      tipoTerminacion: termDef ? termDef.nombre : (v.tipoTerminacion || v.terminacion),
      terminacion: v.terminacion,
      precioGeneral: v.precioGeneral ?? null,
